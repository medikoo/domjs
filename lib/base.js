...
Object.defineProperties(Base.prototype, extend({
	...
	safeCollect: d(function (fn) {
		...
		if (!isDF(result)) result = normalize.call(this.document, result);
	...
	insert: d('e', function (node/*, …nodes*/) {
		var df = normalize.apply(this.document, arguments)
		  , result = toArray(df.childNodes);
		this._current.appendChild(df);
		return result;
	})
}),
